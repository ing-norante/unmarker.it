        ...
        <Card key={step.id} className={cn(
          ...
          <CardContent className="p-4 flex flex-col gap-3">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="neobrutalist" className={cn(
            ...
            <div className="space-y-1">
              <Progress value={step.progress} className="h-3 border-2 border-black" />
              {step.description && (
...
const StepIcon = ({ status }: { status: PipelineStepState['status'] }) => {
  switch (status) {
    case 'done':
      return <div className="bg-black text-white p-1 rounded-full"><Check className="w-4 h-4" /></div>;
    case 'running':
      return <Loader2 className="w-6 h-6 animate-spin" />;
    case 'error':
      return <div className="bg-red-500 text-white p-1 rounded-full"><X className="w-4 h-4" /></div>;
    default:
