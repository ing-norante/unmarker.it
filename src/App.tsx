...
import { PipelineStepState, PipelineStepId } from './lib/types';
import { applyShake, applyStir, applyCrush, DEFAULT_OPTIONS } from './lib/pipeline';
import { ArrowRight, Download, RefreshCcw, Zap } from 'lucide-react';

const INITIAL_STEPS: PipelineStepState[] = [
  { id: 'shake', label: 'Shake (Geometry)', description: 'Micro-rotation & zoom', status: 'idle', progress: 0 },
  { id: 'stir', label: 'Stir (Noise)', description: 'Subtle RGB perturbation', status: 'idle', progress: 0 },
  { id: 'crush', label: 'Crush (Quantization)', description: 'JPEG compression', status: 'idle', progress: 0 },
];
...
function App() {
  ...
  const updateStep = (id: PipelineStepId, update: Partial<PipelineStepState>) => {
  ...
  const processPipeline = async () => {
    ...
    try {
      ...
      setSteps(prev => prev.map(s => s.status === 'running' ? { ...s, status: 'error', error: 'Failed' } : s));
    } finally {
        ...
        <div className="lg:col-span-4 space-y-8">
          <header className="space-y-4">
            ...
            <p className="text-xl font-medium leading-relaxed border-l-4 border-black pl-4">
              ...
              <span className="bg-yellow-300 px-1 font-bold">100% Client-side.</span>
            </p>
          </header>

          <div className="space-y-2">
             <h2 className="text-2xl font-bold uppercase border-b-4 border-black inline-block mb-4">Pipeline</h2>
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {!originalImage && (
            <div className="h-full flex flex-col justify-center">
              <ImageUploader onImageSelect={handleImageSelect} className="h-96" />
            </div>
          )}
          ...
          {originalImage && (
            <div className="grid gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="flex justify-between items-center p-4 bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                 <div className="font-bold truncate max-w-[200px]">{originalImage.name}</div>
                 <div className="flex gap-2">
                    <Button variant="outline" onClick={reset} disabled={isProcessing} className="border-2 border-black rounded-none hover:bg-red-100">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <Card className="neobrutalist-card overflow-hidden bg-white">
                  <div className="bg-yellow-300 border-b-2 border-black p-2 text-center font-bold uppercase text-sm">Original</div>
                ...
                <Card className="neobrutalist-card overflow-hidden bg-white">
                   ...
                   {processedImageUrl && (
                     <div className="p-4 border-t-2 border-black bg-white">
                       <a href={processedImageUrl} download={`shaken-${originalImage.name}`}>
                         <Button className="w-full font-bold border-2 border-black shadow-none hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] rounded-none" size="lg">
