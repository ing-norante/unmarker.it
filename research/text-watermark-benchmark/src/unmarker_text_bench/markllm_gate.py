    ...
    with path.open(encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            try:
                ...
                raise ValueError(f"Missing {error.args[0]!r} at {path}:{line_number}") from error
...
class MarkLLMGateRunner:
    ...
    def _validate_prompts(self) -> None:
        ...
        normalized = [" ".join(prompt.prompt.lower().split()) for prompt in self.prompts]
        if len(normalized) != len(set(normalized)):
            raise ValueError("Prompt texts must be unique after whitespace/case normalization")

        invalid_languages = sorted({prompt.language for prompt in self.prompts} - {"en", "it"})
        ...
        if short:
            raise ValueError(f"Prompts must contain at least 20 characters: {short[:5]}")

        split_counts = Counter((prompt.language, prompt.split) for prompt in self.prompts)
        required = {
        ...
        if missing_cells:
            raise ValueError(f"Every language/split cell needs at least one prompt: {missing_cells}")
    ...
    def run(self, output_dir: Path) -> dict[str, Any]:
        ...
        for prompt in self.prompts:
            ...
            for algorithm in self.config.algorithms:
                ...
                if prompt.split == "evaluation":
                    start = time.perf_counter()
                    watermarked = self.backend.generate_watermarked(algorithm, prompt.prompt, seed)
                ...
                records.append(
                    {
                        ...
                        "markllm_watermarked_detected": (
                            watermarked_detection.detected if watermarked_detection else None
                        ),
        ...
        for record in records:
            ...
            record["calibrated_unwatermarked_detected"] = record["unwatermarked_score"] > threshold
        ...
        self._write_json(
            ...
            {"gate": asdict(self.config), "backend": self.backend.metadata},
        )
        return summary

    def _calibrate_thresholds(self, records: list[dict[str, Any]]) -> dict[str, dict[str, float]]:
            ...
            cells.append(
                {
                    ...
                    "empirical_unwatermarked_fpr": mean(
                        float(item["calibrated_unwatermarked_detected"]) for item in items
                    ),
                    ...
                    "mean_unwatermarked_tokens": mean(item["unwatermarked_tokens"] for item in items),
                    "mean_watermarked_tokens": mean(item["watermarked_tokens"] for item in items),
