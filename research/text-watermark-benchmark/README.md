...
a bilingual XLM-R negative control. Independent human controls fit per-language
1% FPR thresholds and a disjoint control split audits the realized FPR. A
private, unpublished Italian book adds a chapter-grouped human false-positive
stress test without participating in threshold fitting. XLM-R is excluded from
the primary matrix because it fails that stress test.
