...
There are many configuration options, most of which you do not have to ever worry about. This page covers the most relevant ones; the [`PostHogConfig` specification](/docs/references/posthog-js/types/PostHogConfig.md) is generated from the SDK source and lists every supported public option with its description.
...
| before_sendType: FunctionDefault: function () {} | A function that allows you to amend or reject events before they are sent to PostHog. [See below for more information](/docs/libraries/js/usage.md#amending-or-sampling-events) |
| bootstrapType: ObjectDefault: {} | Seeds identity (distinctID, isIdentifiedID), session (sessionID), and feature flag state (featureFlags, featureFlagPayloads) during initialization. See [SDK bootstrapping](/docs/libraries/bootstrapping.md). |
...
| capture_dead_clicksType: Boolean or DeadClicksAutoCaptureConfigDefault: true | Determines if PostHog should automatically capture dead click events. See [dead clicks autocapture](/docs/product-analytics/autocapture.md#dead-clicks-autocapture) for configuration details. |
| capture_exceptionsType: Boolean or ExceptionAutoCaptureConfigDefault: undefined | Determines if PostHog should automatically capture exceptions for [error tracking](/docs/error-tracking.md). When undefined, falls back to the remote configuration from your project settings. |
| capture_heatmapsType: Boolean or HeatmapConfigDefault: undefined | Determines if [heatmap](/docs/toolbar/heatmaps.md) data should be captured. When undefined, falls back to the remote configuration from your project settings. Replaces the deprecated enable_heatmaps option. |
| capture_performanceType: Boolean or PerformanceCaptureConfigDefault: undefined | Determines if performance metrics should be captured: network timing (used by session replay) and [web vitals](/docs/web-analytics/web-vitals.md). When undefined, falls back to the remote configuration from your project settings. If false, neither network timing nor web vitals are captured. |
...
| cookieWinsOnConflictType: BooleanDefault: false (or true when defaults: '2026-08-29' or later) | With localStorage+cookie persistence, makes shared cookie values take precedence over stale per-origin localStorage values. The SDK also synchronizes cookie-backed identity and session changes in open sibling-subdomain tabs before captures and persistence writes. See [persistence](/docs/libraries/js/persistence.md#synchronize-identity-and-sessions-across-subdomains) for details. |
| cookie_expirationType: NumberDefault: 365 | The number of days to store cookies for. |
| cookieless_modeType: Enum: always, on_rejectDefault: undefined | Enables cookieless tracking: PostHog sets no cookies and uses no session or local storage, with user identity handled by a privacy-preserving hash generated on PostHog's servers. Use always to enable it immediately on startup (no cookie banner needed), or on_reject to enable it only when a user rejects cookies. Cookieless mode must also be enabled in your PostHog project settings, otherwise cookieless events are ignored. |
| [custom_blocked_useragents](/docs/libraries/js/usage.md#blocking-bad-actors)Type: ArrayDefault: [] | A list of user agents to block when sending events. |
...
| debugType: BooleanDefault: false | Enables debug mode with more detailed console logging. You can also enable it on a page by appending ?__posthog_debug=true to the URL, or at runtime with posthog.debug(). |
...
| disable_beaconType: BooleanDefault: false | Prevents PostHog from using the browser navigator.sendBeacon API to send events. This replaces the deprecated __preview_disable_beacon option and can reduce $pageleave delivery reliability. |
...
| error_trackingType: ObjectDefault: {} | [Error tracking](/docs/error-tracking.md) options, such as suppression rules for filtering out unwanted exceptions. See ErrorTrackingOptions in the [PostHogConfig specification](/docs/references/posthog-js/types/PostHogConfig.md). |
...
| flag_keysType: Array of StringsDefault: undefined | Feature flag keys to include in this SDK instance's /flags requests. When set, PostHog scopes evaluation and the response to these keys. Dependency flags required to evaluate requested flags may also be evaluated and returned. Leave unset to evaluate all eligible flags. |
...
| persistence_save_debounce_msType: NumberDefault: 0 (or 250 when defaults: '2026-05-30' or later) | Debounces rapid persistence writes to localStorage or cookies. In-memory state updates immediately, and pending writes flush on page unload. |
| split_storageType: BooleanDefault: false (or true when defaults: '2026-05-30' or later) | Stores large feature flag and survey payloads in separate localStorage entries instead of the main persistence blob. Applies to localStorage and localStorage+cookie persistence. |
| detect_google_search_appType: BooleanDefault: false (or true when defaults: '2026-05-30' or later) | Detects Google Search App webviews as $browser: 'Google Search App' instead of attributing them to Mobile Safari or Chrome. |
| Attribute | Description |
| --- | --- |
| property_denylistType: ArrayDefault: [] | A list of properties that should never be sent with capture calls. |
| person_profilesType: Enum: always, identified_only, neverDefault: identified_only | Set whether events should capture identified events and process person profiles. |
| reuseAnonymousIdType: BooleanDefault: false | When true, identify() does not send the previous anonymous ID as $anon_distinct_id, so events captured before identify() are not merged into the identified person. Useful with memory or sessionStorage persistence when your app already has a stable user ID. |
...
| logsType: ObjectDefault: undefined | Configuration options for browser logs. See [configuring logs](#configuring-logs) below. |
...
| surveysType: ObjectDefault: undefined | [Survey](/docs/surveys.md)-specific configuration: prefillFromUrl (prefill responses from URL parameters), autoSubmitIfComplete, and autoSubmitDelay. |
...
The proxy routes must preserve query parameters and match every path that the function returns. See the [self-hosted proxy reference](/docs/advanced/proxy/proxy-reference.md) for upstream hosts and other requirements.
...
| content_ignorelistType: Boolean or Array of StringsDefault: true when defaults: '2025-11-30' or later | Excludes elements from rage click tracking based on their text content or aria-label. When true, uses default keywords ['next', 'previous', 'prev', '>', '<']. With defaults: '2026-05-30' or later, the default list also excludes +, -, −, and –, and text-selection surfaces are ignored. When set to an array of strings (max 10 items), uses custom keywords. Set to false to disable content-based exclusion. Useful for preventing false positives on navigation elements. |
| ignore_text_selectionType: BooleanDefault: false (true when defaults: '2026-05-30' or later) | Excludes text-editing surfaces (textareas, text-like inputs, and contenteditable elements) from rage click detection, since rapid repeated clicks there are usually double- or triple-click text selection rather than rage. |
| threshold_pxType: NumberDefault: 30 | Maximum pixel distance between consecutive clicks for them to still count toward a rage click. |
| click_countType: NumberDefault: 3 | Number of consecutive clicks within the timeout required to trigger a rage click. |
| timeout_msType: NumberDefault: 1000 | Maximum time (in milliseconds) allowed between consecutive clicks. |

## Configuring logs

The `logs` config controls how browser log records are captured and sent to PostHog via OpenTelemetry. You can send structured records with `posthog.logger` (or `posthog.captureLog` for full control), autocapture `console.*` calls, or both. See the [JavaScript web Logs installation guide](/docs/logs/installation/javascript.md) for a walkthrough.

| Attribute | Description |
| --- | --- |
| captureConsoleLogsType: BooleanDefault: undefined | When enabled, automatically captures console.log, console.warn, console.error, and friends, and sends them to PostHog as structured log records. Takes effect from init(), including calls made before the logs script finishes loading. Independent of the Capture console logs project setting – either one enables capture, and the project setting does not switch this option off. Review your app's console output before enabling – overriding console can forward passwords, tokens, or user input written to the console by your code or by third-party libraries. |
| serviceNameType: StringDefault: 'unknown_service' | The service name applied to all log records. Maps to the [OpenTelemetry resource attribute](https://opentelemetry.io/docs/specs/semconv/resource/) service.name. Set this to distinguish records from different frontend apps or environments when viewing them in PostHog. |
| environmentType: StringDefault: undefined | Deployment environment, e.g. 'production' or 'staging'. Maps to the OpenTelemetry resource attribute deployment.environment. |
| serviceVersionType: StringDefault: undefined | Release version, e.g. '1.2.3'. Maps to the OpenTelemetry resource attribute service.version. |
| resourceAttributesType: ObjectDefault: undefined | Additional OpenTelemetry resource attributes applied to every record, e.g. { 'host.name': 'web-01' }. Named fields above are applied first and can be overridden here. |
| flushIntervalMsType: NumberDefault: 3000 | How often batched log records are flushed, in milliseconds. |
| maxBufferSizeType: NumberDefault: 100 | Buffer size before forcing a flush. |
| maxLogsPerIntervalType: NumberDefault: 1000 | Rate limit per flush window. Excess records are dropped with a single warning, protecting against runaway loggers flooding the network. |
...
| advanced_feature_flags_dedup_per_sessionType: BooleanDefault: false | Scopes $feature_flag_called deduplication to the current session instead of per identity. Useful for [experiments](/docs/experiments.md) where a flag was evaluated before the experiment launched. Without this, returning users won't generate new [exposures](/docs/experiments/exposures.md). Mobile SDKs already deduplicate per session by default. |
